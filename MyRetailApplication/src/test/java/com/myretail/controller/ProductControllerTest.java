...
public class ProductControllerTest {
	...
	public void testFindByValidId() throws Exception {
		...
		this.mvc.perform(get("/v1/products/78842191").contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON))
				.andExpect(jsonPath("$.product.id").value("78842191"))
				.andExpect(jsonPath("$.product.name").value("Men's Big & Tall Standard Fit V-Neck Sweater - Goodfellow & Co™"))
	...
	public void testUpdatePriceValidId() throws Exception {
		...
		this.mvc.perform(put("/v1/products/53494594").content("{\"value\":120.49,\"currency_code\":\"USD\"}")
